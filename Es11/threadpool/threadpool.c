...
threadpool_t *createThreadPool(int numthreads,int pending_size){
	...
	//Alloco i thread e la task queue
	...
	if(!pool->pending_queue){
		free(pool->threads);
		free(pool);
		return NULL;
	}

	//Init of mutex and cond variables
...
int addToThreadPool(threadpool_t *pool, void (*f)(void *), void *arg){
	...
	//Se i task in esecuzione sono piu del numero di thread
	if(pool->taskonthefly >= pool->numthreads){
		//Se non gestisco i task pendenti esco
		if(nopending){
			assert(pool->count == 0);
	...
	//Altrimenti posso mettere il task in coda pendenti
 ...
 * @brief funzione eseguita dal thread worker detached(che non appartiene al pool)
 */
